...
class LaporanKlinikController extends Controller
{
    ...
    public function preview(Request $request){
        ...
        if($var['jenis'] == 'Vaksinasi'){
           ...
           /*  $begin = new DateTime($request->dari_tanggal);
            ...
            $var['klinik'] = KlinikTerapi::select("klinik_terapi.*","klinik.sub_satuan_kerja_id","klinik.pemilik_id","klinik.spesies_id","klinik.nama_hewan","penyakit.penyakit")->join('klinik','klinik_terapi.klinik_id','=','klinik.id')->join('pemilik','klinik.pemilik_id','=','pemilik.id')->join("penyakit","klinik_terapi.diagnosa","penyakit.id")->where("klinik_terapi.no_pasien","like","%SMG%")->whereBetween('klinik_terapi.tanggal_periksa',[$var['dari_tanggal'],$var['sampai_tanggal']])->orderBy("tanggal_periksa")->get();
    ...
    public function cetakLaporanPasien(Request $request)
    {
        ...
        if($var['jenis'] == 'Vaksinasi'){
            ...
            $var['klinik'] = KlinikTerapi::select("klinik_terapi.*","klinik.sub_satuan_kerja_id","klinik.pemilik_id","klinik.spesies_id","klinik.nama_hewan","penyakit.penyakit")->join('klinik','klinik_terapi.klinik_id','=','klinik.id')->join('pemilik','klinik.pemilik_id','=','pemilik.id')->join("penyakit","klinik_terapi.diagnosa","penyakit.id")->where("klinik_terapi.no_pasien","like","%SMG%")->whereBetween('klinik_terapi.tanggal_periksa',[$var['dari_tanggal'],$var['sampai_tanggal']])->get();
            $var['obat'] = Obat::get()->where("klinik",1);
            $var['operasi'] = Operasi::where("kode","OPR")->orWhere("kode","MEDIVET1")->get();
			$var['vaksin'] = Obat::get()->where("klinik",2);
			$var['tarif'] = array();
			...
			foreach($var['klinik'] as $klinik){
				...
				$var['tarif'][$klinik->id] = Pembayaran::get()->where('klinik_terapi_id',$klinik->id)->first()->total;
			}
        ...
        if($request->format == 'PDF'){
            ...
            header("Expires: 0");
            return view('laporan.klinik.laporan-klinik', compact('var','jumlah','jumlah_operasi','subSatuanKerja', 'spesies', 'helper','data_obat'));
        }
    }
}
