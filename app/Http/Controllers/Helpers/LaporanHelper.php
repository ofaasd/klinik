...
class LaporanHelper
{
    ...
    {        
            if($jenis == 'Vaksinasi'){
                if($spesiesId == 'ayam'){
					...
					$jumlah = KlinikTerapi::join('klinik_dosis','klinik_terapi.id','=','klinik_dosis.klinik_id')->join('klinik','klinik_terapi.klinik_id','=','klinik.id')->where('klinik.spesies_id',$spesiesId)->whereBetween('klinik_terapi.tanggal_periksa',[$dariTanggal,$sampaiTanggal])->where('klinik_dosis.tindakan',1)->where('klinik_dosis.terapi_id', $terapiId)->count();
					return $jumlah;
                }                                
	...
	public function jumlahpad($tanggal){
		$queryJumlah = DB::table("klinik_terapi")->join("pembayaran","klinik_terapi.id","pembayaran.klinik_terapi_id")->where("tanggal_periksa",$tanggal)->sum("pembayaran.total");
		return $queryJumlah;
	}
	public function detailpad($tanggal){
		$queryJumlah = DB::table("klinik_terapi")->join("pembayaran","klinik_terapi.id","pembayaran.klinik_terapi_id")->where("tanggal_periksa",$tanggal)->get();
		return $queryJumlah;
	}
